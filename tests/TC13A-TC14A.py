...
# A user wants to like and dislike a comment in a post. 
...
MOCK_COMMENT = "This is another comment from Selenium"
...
if MOCK_COMMENT in driver.page_source:
    print("Comment was posted to give him like/dislike")
else:
...
column = driver.find_element(By.XPATH, "/html/body/div[2]/div[1]")
articles = column.find_elements(By.XPATH, "//article[@class='media']")
for article in articles:
    if MOCK_COMMENT in article.text:
        # save like count
        like_count = int(article.find_element(By.XPATH, ".//h1").text)
        elem = article.find_element(By.XPATH, ".//a[contains(text(), 'Like')]")
        elem.click()
        time.sleep(1)
...
for article in articles:
    if MOCK_COMMENT in article.text:
        if int(article.find_element(By.XPATH, ".//h1").text) != like_count:
          print("LIKE COMMENT IS PASSED")
        else:
          print("LIKE COMMENT IS FAILED")
          time.sleep(5)
          exit(0) 
        like_count = int(article.find_element(By.XPATH, ".//h1").text)
        elem = article.find_element(By.XPATH, ".//a[contains(text(), 'Dislike')]")
        elem.click()
        time.sleep(1)
        print("Dislike button is clicked")
        break
...
for article in articles:
    if MOCK_COMMENT in article.text:
        if int(article.find_element(By.XPATH, ".//h1").text) != like_count:
          print('DISLIKE COMMENT IS PASSED')
        else:
          print('DISLIKE COMMENT IS FAILED')
          time.sleep(5)
          exit(0)
        break
